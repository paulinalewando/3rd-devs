...
export class AudioService {
    ...
    return new Promise((resolve, reject) => {
      ffmpeg(filePath)
        ...
        .on("stderr", (stderrLine) => {
          const rmsMatch = stderrLine.match(
  ...
  async getAverageSilenceDuration(filePath: string): Promise<number> {
    const averageSilenceThreshold =
      await this.getAverageSilenceThreshold(filePath);
    const silenceSegments = await this.detectSilence(
      filePath,
      averageSilenceThreshold + 25,
      1
    );
  ...
  async processAndSaveNonSilentChunks(filePath: string): Promise<string[]> {
    ...
    const nonSilentChunks = this.extractNonSilentChunks(
      silenceIntervals,
      metadata.duration
    );
  ...
  async split(
    filePath: string,
    silenceThresholdOffset = 25
  ): Promise<string[]> {
    const minSilenceDuration =
      (await this.getAverageSilenceDuration(filePath)) * 0.9;
    const averageSilenceThreshold =
      await this.getAverageSilenceThreshold(filePath);
    let nonSilentChunks = await this.detectNonSilence(
      filePath,
      averageSilenceThreshold + silenceThresholdOffset,
      minSilenceDuration
    );
