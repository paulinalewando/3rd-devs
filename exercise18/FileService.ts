...
export class FileService {
  ...
  async save(
    ...
    try {
      ...
      if (source) {
        ...
        relativeSource = path.relative(
          this.STORAGE_DIR,
          path.join(this.STORAGE_DIR, source)
        );
      }
  ...
  async getMimeType(filePath: string): Promise<string> {
    try {
      ...
      const absolutePath = path.isAbsolute(filePath)
        ? filePath
        : path.join(this.STORAGE_DIR, filePath);
  ...
  private async getPlainFileContentsFromDrive(
    ...
    try {
      ...
      console.error(`Failed to get file contents from Drive: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async takeScreenshot(filePath: string, fileName: string): Promise<string[]> {
    try {
      ...
      if ([".doc", ".docx", ".xls", ".xlsx"].includes(extension)) {
        const absoluteFilePath = path.isAbsolute(filePath)
          ? filePath
          : path.join(this.STORAGE_DIR, filePath);
        const { pdfPath: generatedPdfPath } =
          await this.processOfficeFile(absoluteFilePath);
        pdfPath = generatedPdfPath;
        if ([".xls", ".xlsx"].includes(extension)) {
          options = { ...options, width: 3508, height: 2480 };
        }
      } else if (extension === ".pdf") {
        pdfPath = path.isAbsolute(filePath)
          ? filePath
          : path.join(this.STORAGE_DIR, filePath);
      } else {
        throw new Error(
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        const outputPath = join(this.TEMP_DIR, `${outputBaseName}_${i}.jpg`);
  ...
  private async getPageCount(pdfPath: string): Promise<number> {
    ...
    try {
      const absolutePdfPath = path.isAbsolute(pdfPath)
        ? pdfPath
        : path.join(this.STORAGE_DIR, pdfPath);
  ...
  async fetchAndSaveUrlFile(
    ...
    try {
      ...
      if (!fileExtension) {
        const scrapedContent = await this.webSearchService.scrapeUrls(
          [url],
          fileUUID
        );
  ...
  async process(
    ...
    if (
      ...
      try {
        const savedFile = await this.fetchAndSaveUrlFile(
          filePathOrUrl,
          fileUUID
        );
      ...
      originalPath = filePathOrUrl;
      const adjustedPath = filePathOrUrl.startsWith("storage/")
        ? filePathOrUrl.slice(8)
        : filePathOrUrl;
      const fileContent = await fs.readFile(
        path.join(this.STORAGE_DIR, adjustedPath)
      );
      const mimeType = await this.getMimeTypeFromBuffer(
        fileContent,
    ...
    const processContent = async (
      ...
      if (chunkSize) {
        ...
        return [
          await this.textService.document(content, undefined, {
            ...metadata,
            uuid: uuidv4(),
          }),
        ];
