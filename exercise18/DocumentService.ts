...
export class DocumentService {
    ...
    const hybridResults = await Promise.all(
      queries.map(async (query: { natural: string; search: string }) => {
        ...
        return results.map((doc) => ({
          ...
          metadata: {
            ...doc.metadata,
            description: `This document was found by searching for the query: "${query.natural}"`,
          },
        }));
      })
    );
  ...
  async summarize(documents: IDoc[]): Promise<IDoc> {
    ...
    const extractedData = extractionTypes.reduce(
      (acc, { key }, index) => {
  ...
  async translate(
    ...
    for (const doc of documents) {
      ...
      const translatedDocWithPlaceholders =
        this.textService.restorePlaceholders({
          ...doc,
          text: translatedText,
          metadata: {
            ...doc.metadata,
      ...
      const standardizedMetadata = generateMetadata({
        ...
        conversation_uuid:
          translatedDocWithPlaceholders.metadata.conversation_uuid,
        additional: {
