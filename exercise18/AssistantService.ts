...
export class AssistantService {
  ...
  async understand(
    ...
    return JSON.parse(
      understanding.choices[0].message.content as string
    ) as IPlan;
  }
  ...
  async answer(
    ...
    const unwrappedContext = context.map((doc) =>
    ...
    const response = (await this.openaiService.completion({
      messages: [
        {
          role: "system",
          content: answerPrompt({ context: unwrappedContext, uploads, query }),
        },
        ...messages,
      ],
      model: "gpt-4o",
  ...
  async websearch(query: string, conversation_uuid: string): Promise<IDoc[]> {
    ...
    const webSearchResults = await this.webSearchService.search(
      query,
      conversation_uuid
    );
    Promise.all(
      webSearchResults.map(async (doc) => {
  ...
  async write(
    ...
    const document = (await this.openaiService.completion({
      ...
      messages: [
        {
          role: "system",
          content: writePrompt({ documents: unwrappedContext }),
        },
        { role: "user", content: query },
      ],
      jsonMode: true,
    ...
    const contentWithContext = content.replace(
      /\[\[([^\]]+)\]\]/g,
  ...
  async upload(doc: IDoc) {
    ...
    await this.databaseService.insertDocument(doc);
    return this.fileService.saveDocsToFile(
      [doc],
      doc.metadata.name || "file.md"
    );
  }
  ...
  async processDocument(
    ...
    for (const action of processActions) {
      ...
      const chunkSize = ["translate", "synthesize"].includes(action.type)
        ? 3500
        : undefined;
      ...
      let documents: IDoc[] =
        source.startsWith("[[") && source.endsWith("]]")
          ...
          : (
              await this.fileService.process(
                source,
                chunkSize,
                conversation_uuid
              )
      ...
      switch (action.type) {
        case "translate":
          result = await this.documentService.translate(
            documents,
            action.original_language,
        ...
        case "synthesize":
          result = [
            await this.documentService.synthesize(action.query, documents),
          ];
          break;
        case "extract":
          result = await this.documentService.extract(
            documents,
            action.extraction_type,
            action.description
          );
          break;
        case "answer":
          result = [
            await this.documentService.answer(action.question, documents),
          ];
          break;
        default:
