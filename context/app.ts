...
const completion = (await openAIService.completion({
  messages: [
    {
      role: "system",
      content: `As an AI assistant, you can use the following documents in your responses by referencing them with the placeholder: [[uuid]] (double square brackets).
...
const answer =
  completion.choices[0].message.content?.replace(
    /\[\[([^\]]+)\]\]/g,
    (match, uuid) => documents[uuid] || match
  ) || "";
