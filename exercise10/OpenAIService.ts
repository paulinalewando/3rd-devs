...
export class OpenAIService {
  ...
  /**
   * Describes what's visible in an image using GPT-4o vision capabilities
   */
  async describeImage(filePath: string, context?: string): Promise<string> {
    try {
      const imageBuffer = await readFile(filePath);
      const base64Image = imageBuffer.toString("base64");
      ...
      const contextPrompt = context
        ? `\n\nKontekst: ${context}`
        : "\n\nKontekst: Obraz z artykułu naukowego o podróżach w czasie i eksperymentach z transmisją materii.";

      const messages: ChatCompletionMessageParam[] = [
        {
          role: "system",
          content: `Jesteś ekspertem analizującym obrazy. Opisz szczegółowo co widzisz na obrazie. Zwróć szczególną uwagę na:
- Obiekty (szczególnie owoce, jedzenie, przedmioty)
- Tekst (jeśli jest widoczny)
- Miejsca i budynki
- Ludzi i ich działania
- Wszelkie szczegóły które mogą być istotne w kontekście naukowym

Odpowiadaj w języku polskim.${contextPrompt}`,
        },
        {
          role: "user",
          content: [
      ...
      console.error(`Error describing image ${filePath}:`, error);
      throw error;
    }
  }
