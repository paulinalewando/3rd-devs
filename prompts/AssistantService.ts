...
export class AssistantService {
  ...
  async answer(
    ...
    const compiledPrompt = this.langfuseService.compilePrompt(prompt, {});
    if (typeof compiledPrompt === "string") {
      throw new Error("Expected chat prompt but got text prompt");
    }
    const [systemMessage] = compiledPrompt;
    const thread = [
      systemMessage,
